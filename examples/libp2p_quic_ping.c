...
int main(void) {
    ...
    COPY(out_pkt.dcid, dcid, 8); out_pkt.dcid_len = 8;
    COPY(out_pkt.scid, scid, 8); out_pkt.scid_len = 8;
    out_pkt.pkt_num = 0;
    out_pkt.pn_length = 1;
    ...
    if (in_pkt.pkt_type != QUIC_PT_INITIAL) { fprintf(stderr, "wrong pkt type\n"); return 1; }
    if (in_pkt.pkt_num != 0) { fprintf(stderr, "wrong pn\n"); return 1; }
    if (memcmp(in_pkt.dcid, dcid, 8) != 0) { fprintf(stderr, "dcid mismatch\n"); return 1; }
    if (memcmp(in_pkt.scid, scid, 8) != 0) { fprintf(stderr, "scid mismatch\n"); return 1; }
    ...
    uint64_t off, l;
    if (speer_qf_r_varint(&fr, &off) != 0 ||
        speer_qf_r_varint(&fr, &l) != 0) {
    ...
    const uint8_t* fdata;
    if (speer_qf_r_bytes(&fr, &fdata, (size_t)l) != 0) { fprintf(stderr, "frame data fail\n"); return 1; }
