...
static void pt_set_zero(pt_t* p) {
    speer_bn_zero(&p->x);
    speer_bn_zero(&p->y); p->y.limbs[0] = 1; p->y.n = 1;
    speer_bn_zero(&p->z);
    p->infinity = 1;
}
...
static void pt_double(pt_t* r, const pt_t* p) {
    ...
    if (speer_bn_is_zero(&two_y)) { pt_set_zero(r); return; }
...
static void pt_add(pt_t* r, const pt_t* p, const pt_t* q) {
    ...
    if (speer_bn_cmp(&p->x, &q->x) == 0) {
        ...
        if (speer_bn_is_zero(&sum)) { pt_set_zero(r); return; }
        pt_double(r, p);
        return;
    }
...
static void pt_scalar_mul(pt_t* r, const pt_t* base, const speer_bn_t* k) {
    pt_t res; pt_set_zero(&res);
    pt_t cur = *base;
    size_t bits = speer_bn_bit_size(k);
    for (size_t i = 0; i < bits; i++) {
        if (speer_bn_get_bit(k, i)) {
            pt_t t; pt_add(&t, &res, &cur); res = t;
        }
        pt_t t; pt_double(&t, &cur); cur = t;
    }
    *r = res;
}

int speer_ecdsa_p256_verify(const uint8_t pubkey[64],
