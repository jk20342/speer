...
void speer_aes_set_encrypt_key(speer_aes_key_t* k, const uint8_t* key, size_t key_bits) {
    int nk;
    if (key_bits == 128) { nk = 4; k->nr = 10; }
    else if (key_bits == 192) { nk = 6; k->nr = 12; }
    else if (key_bits == 256) { nk = 8; k->nr = 14; }
    else { k->nr = 0; return; }

    int total = 4 * (k->nr + 1);
    for (int i = 0; i < nk; i++) {
        k->round_keys[i] = LOAD32_BE(key + i * 4);
    }
...
void speer_aes_encrypt(const speer_aes_key_t* k, const uint8_t in[16], uint8_t out[16]) {
    ...
    for (int round = 1; round < nr; round++) {
        t0 = te0[(s0 >> 24) & 0xff] ^ TE1((s1 >> 16) & 0xff) ^ TE2((s2 >> 8) & 0xff) ^ TE3(s3 & 0xff) ^ rk[4*round];
        t1 = te0[(s1 >> 24) & 0xff] ^ TE1((s2 >> 16) & 0xff) ^ TE2((s3 >> 8) & 0xff) ^ TE3(s0 & 0xff) ^ rk[4*round + 1];
        t2 = te0[(s2 >> 24) & 0xff] ^ TE1((s3 >> 16) & 0xff) ^ TE2((s0 >> 8) & 0xff) ^ TE3(s1 & 0xff) ^ rk[4*round + 2];
        t3 = te0[(s3 >> 24) & 0xff] ^ TE1((s0 >> 16) & 0xff) ^ TE2((s1 >> 8) & 0xff) ^ TE3(s2 & 0xff) ^ rk[4*round + 3];
