...
static int rsa_public_op(uint8_t* out, size_t out_len,
                          const uint8_t* sig, size_t sig_len,
                          const uint8_t* n_be, size_t n_len,
                          const uint8_t* e_be, size_t e_len) {
    speer_bn_t n, e, s, m;
...
int speer_rsa_pkcs1_v15_verify(const uint8_t* n, size_t n_len,
    ...
    if (em[0] != 0x00 || em[1] != 0x01) { free(em); return -1; }
    size_t i = 2;
    while (i < sig_len && em[i] == 0xff) i++;
    if (i >= sig_len || em[i] != 0x00) { free(em); return -1; }
    if (i - 2 < 8) { free(em); return -1; }
    i++;
    
    const uint8_t* prefix;
    size_t prefix_len;
    if (hash == &speer_hash_sha256) { prefix = sha256_oid; prefix_len = sizeof(sha256_oid); }
    else if (hash == &speer_hash_sha384) { prefix = sha384_oid; prefix_len = sizeof(sha384_oid); }
    else if (hash == &speer_hash_sha512) { prefix = sha512_oid; prefix_len = sizeof(sha512_oid); }
    else { free(em); return -1; }
    
    if (sig_len - i != prefix_len + msg_hash_len) { free(em); return -1; }
    if (!EQUAL(em + i, prefix, prefix_len)) { free(em); return -1; }
    if (!EQUAL(em + i + prefix_len, msg_hash, msg_hash_len)) { free(em); return -1; }
    
    free(em);
    return 0;
}

static void mgf1(uint8_t* out, size_t out_len, const uint8_t* seed, size_t seed_len, const speer_hash_iface_t* hash) {
...
int speer_rsa_pss_verify(const uint8_t* n, size_t n_len,
                          const uint8_t* e, size_t e_len,
                          const speer_hash_iface_t* hash,
                          const uint8_t* msg_hash, size_t msg_hash_len,
                          const uint8_t* sig, size_t sig_len,
    ...
    if (em_len < hash_len + salt_len + 2) { free(em); return -1; }
    if (em[em_len - 1] != 0xbc) { free(em); return -1; }
    
    size_t db_len = em_len - hash_len - 1;
    uint8_t* db_mask = (uint8_t*)malloc(db_len);
    if (!db_mask) { free(em); return -1; }
    ...
    for (size_t i = 0; i < db_len - salt_len - 1; i++) {
        if (em[i] != 0) { free(em); return -1; }
    }
    if (em[db_len - salt_len - 1] != 0x01) { free(em); return -1; }
    
    uint8_t mprime[8 + SPEER_HASH_MAX_DIGEST + 256];
    if (salt_len > 256) { free(em); return -1; }
    ZERO(mprime, 8);
