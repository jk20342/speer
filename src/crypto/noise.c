...
static void encrypt_and_hash(speer_handshake_t* hs, uint8_t* out, const uint8_t* plaintext, size_t len) {
    uint8_t nonce[12] = {0};
...
static int decrypt_and_hash(speer_handshake_t* hs, uint8_t* out, const uint8_t* ciphertext, size_t len) {
    if (len < 16) return -1;
