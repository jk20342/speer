...
static int aes_gcm_seal_n(size_t key_bits,
                          const uint8_t* key,
                          const uint8_t* nonce,
    ...
    uint8_t ctr[16];
    COPY(ctr, j0, 16);
    for (int i = 15; i >= 12; i--) { ctr[i]++; if (ctr[i] != 0) break; }
...
static int aes_gcm_open_n(size_t key_bits,
                          const uint8_t* key,
                          const uint8_t* nonce,
                          const uint8_t* aad, size_t aad_len,
...
static int aes128_gcm_seal(const uint8_t* k, const uint8_t* n,
                            const uint8_t* aad, size_t al,
                            const uint8_t* p, size_t pl,
                            uint8_t* oc, uint8_t* ot) {
    return aes_gcm_seal_n(128, k, n, aad, al, p, pl, oc, ot);
}
static int aes128_gcm_open(const uint8_t* k, const uint8_t* n,
                            const uint8_t* aad, size_t al,
                            const uint8_t* c, size_t cl,
                            const uint8_t* t, uint8_t* op) {
    return aes_gcm_open_n(128, k, n, aad, al, c, cl, t, op);
}
static int aes256_gcm_seal(const uint8_t* k, const uint8_t* n,
                            const uint8_t* aad, size_t al,
                            const uint8_t* p, size_t pl,
                            uint8_t* oc, uint8_t* ot) {
    return aes_gcm_seal_n(256, k, n, aad, al, p, pl, oc, ot);
}
static int aes256_gcm_open(const uint8_t* k, const uint8_t* n,
                            const uint8_t* aad, size_t al,
                            const uint8_t* c, size_t cl,
                            const uint8_t* t, uint8_t* op) {
...
const speer_aead_iface_t speer_aead_aes128_gcm = {
    .name = "aes-128-gcm",
    .key_len = 16, .nonce_len = 12, .tag_len = 16,
    .seal = aes128_gcm_seal, .open = aes128_gcm_open
};
const speer_aead_iface_t speer_aead_aes256_gcm = {
    .name = "aes-256-gcm",
    .key_len = 32, .nonce_len = 12, .tag_len = 16,
    .seal = aes256_gcm_seal, .open = aes256_gcm_open
};
