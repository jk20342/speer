...
int speer_libp2p_noise_init(speer_libp2p_noise_t* n,
...
int speer_libp2p_noise_payload_make(uint8_t* out, size_t cap, size_t* out_len,
    ...
    if (speer_libp2p_pubkey_proto_encode(pubkey_proto, sizeof(pubkey_proto),
                                           kt, libp2p_pub, libp2p_pub_len,
                                           &pubkey_proto_len) != 0) return -1;
...
int speer_libp2p_noise_payload_parse(const uint8_t* in, size_t in_len,
                                      speer_libp2p_keytype_t* kt,
                                      const uint8_t** libp2p_pub, size_t* libp2p_pub_len,
                                      const uint8_t** sig, size_t* sig_len) {
    speer_pb_reader_t r;
    ...
    while (r.pos < r.len) {
        ...
        if (f == 1 && wire == PB_WIRE_LEN) {
            ...
            if (speer_libp2p_pubkey_proto_decode(d, l, kt, libp2p_pub, libp2p_pub_len) != 0) return -1;
            got_id = 1;
...
int speer_libp2p_noise_verify_static(speer_libp2p_keytype_t kt,
                                      const uint8_t* libp2p_pub, size_t libp2p_pub_len,
                                      const uint8_t static_pub[32],
                                      const uint8_t* sig, size_t sig_len) {
    if (kt != SPEER_LIBP2P_KEY_ED25519) return -1;
...
int speer_libp2p_noise_seal(speer_libp2p_noise_t* n,
                              const uint8_t* plaintext, size_t pt_len,
                              uint8_t* out_ct, size_t* out_ct_len) {
    uint8_t nonce[12] = {0};
    STORE64_LE(nonce + 4, n->send_nonce);
    uint8_t tag[16];
    if (speer_aead_chacha20_poly1305.seal(n->send_key, nonce,
                                           NULL, 0,
                                           plaintext, pt_len,
                                           out_ct, tag) != 0) return -1;
...
int speer_libp2p_noise_open(speer_libp2p_noise_t* n,
                              const uint8_t* ct, size_t ct_len,
                              uint8_t* out_pt, size_t* out_pt_len) {
    if (ct_len < 16) return -1;
    uint8_t nonce[12] = {0};
    STORE64_LE(nonce + 4, n->recv_nonce);
    if (speer_aead_chacha20_poly1305.open(n->recv_key, nonce,
                                           NULL, 0,
                                           ct, ct_len - 16,
                                           ct + ct_len - 16,
                                           out_pt) != 0) return -1;
