...
void speer_yamux_init(speer_yamux_session_t* s, int is_initiator,
                      ...
                      int (*recv_raw)(void*, uint8_t*, size_t, size_t*),
                      void* user) {
    ZERO(s, sizeof(*s));
...
static int send_frame(speer_yamux_session_t* s, const speer_yamux_hdr_t* h, const uint8_t* body, size_t body_len) {
    uint8_t hdr[12];
...
int speer_yamux_stream_write(speer_yamux_session_t* s, speer_yamux_stream_t* st, const uint8_t* data, size_t len) {
    while (len > 0) {
...
int speer_yamux_send_window_update(speer_yamux_session_t* s, speer_yamux_stream_t* st, uint32_t inc) {
    speer_yamux_hdr_t h = {
        .version = YAMUX_VERSION,
        .type = YAMUX_TYPE_WINDOW_UPDATE,
        .flags = 0,
...
int speer_yamux_pump(speer_yamux_session_t* s) {
    ...
    if (h.type == YAMUX_TYPE_DATA) {
        ...
        if (h.length > 0 && st) {
            ...
            uint8_t* tmp = (uint8_t*)malloc(h.length);
            if (tmp) { s->recv_raw(s->user, tmp, h.length, &got); free(tmp); }
        }
        if (st && (h.flags & YAMUX_FLAG_FIN)) st->remote_closed = 1;
        if (st && (h.flags & YAMUX_FLAG_RST)) { st->reset = 1; st->remote_closed = 1; }
        ...
        if (h.flags & YAMUX_FLAG_SYN) {
            speer_yamux_send_ping(s, h.length, 1);
        }
