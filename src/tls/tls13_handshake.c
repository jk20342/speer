...
static int append_transcript(speer_tls13_t* h, uint8_t msg_type, const uint8_t* body, size_t body_len) {
...
int speer_tls13_init_handshake(speer_tls13_t* h, speer_tls_role_t role,
    ...
    if (speer_libp2p_pubkey_proto_encode(h->libp2p_pubkey_proto, sizeof(h->libp2p_pubkey_proto),
                                           ...
                                           &h->libp2p_pubkey_proto_len) != 0) return -1;
...
static int build_client_hello(speer_tls13_t* h) {
    ...
    append_transcript(h, TLS_HS_CLIENT_HELLO,
                       h->out_buf + 4, hs_body_len);
    return 0;
}
...
int speer_tls13_handshake_consume(speer_tls13_t* h,
    ...
    switch (h->state) {
        case TLS_ST_WAIT_SH:
            if (msg_type != TLS_HS_SERVER_HELLO) { h->state = TLS_ST_ERROR; return SPEER_TLS_ERR; }
            if (parse_server_hello(h, body, body_len) != 0) { h->state = TLS_ST_ERROR; return SPEER_TLS_ERR; }
            h->transcript_len -= 4 + body_len;
            append_transcript(h, msg_type, body, body_len);
            if (derive_handshake_keys(h) != 0) { h->state = TLS_ST_ERROR; return SPEER_TLS_ERR; }
        ...
        case TLS_ST_WAIT_FINISHED: {
            ...
            speer_tls13_application_keys(&h->ks, &h->client_app_keys, &h->server_app_keys, hs_hash_after);
            h->state = TLS_ST_DONE;
