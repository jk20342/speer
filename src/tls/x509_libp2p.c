        ...
        if (node.tag == 0xa3) {
            ...
            while (ec < ee) {
                ...
                if (!speer_asn1_oid_eq(&oid, (const uint8_t*)LIBP2P_TLS_EXT_OID_BYTES,
                                         LIBP2P_TLS_EXT_OID_LEN)) continue;
...
static int parse_signed_libp2p_key(const uint8_t* in, size_t in_len,
                                     speer_x509_libp2p_t* out) {
    speer_asn1_t seq;
    ...
    if (speer_libp2p_pubkey_proto_decode(pkey.value, pkey.value_len,
                                            &out->keytype,
                                            (const uint8_t**)&pkey.value,
                                            &pkey.value_len) != 0) return -1;
...
int speer_x509_libp2p_parse(speer_x509_libp2p_t* out, const uint8_t* der, size_t der_len) {
    ...
    const uint8_t* ext_value; size_t ext_len;
...
int speer_x509_libp2p_make_self_signed(uint8_t* out, size_t cap, size_t* out_len,
    ...
    (void)cert_priv_key; (void)cert_pub_key; (void)cap; (void)out;
    (void)libp2p_kt; (void)libp2p_pub; (void)libp2p_pub_len;
    (void)libp2p_priv; (void)libp2p_priv_len;
    if (out_len) *out_len = 0;
    (void)put_len; (void)emit_tlv;
    return -1;
}
