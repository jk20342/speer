...
#define SWAP32(a, b) do { uint32_t tmp[32]; COPY(tmp, a, sizeof(tmp)); COPY(a, b, sizeof(tmp)); COPY(b, tmp, sizeof(tmp)); } while (0)
...
#define LOAD64_LE(p) \
    (((uint64_t)LOAD32_LE(p)) | \
     ((uint64_t)LOAD32_LE((p) + 4) << 32))
...
int speer_aes_gcm_encrypt(uint8_t* out, const uint8_t* in, size_t len, 
                          const uint8_t key[32], const uint8_t iv[12],
                          const uint8_t* aad, size_t aad_len,
                          uint8_t tag[16]);
int speer_aes_gcm_decrypt(uint8_t* out, const uint8_t* in, size_t len,
                          const uint8_t key[32], const uint8_t iv[12],
                          const uint8_t* aad, size_t aad_len,
...
int speer_noise_xx_init(speer_handshake_t* hs, 
...
void speer_noise_xx_split(speer_handshake_t* hs, 
                          uint8_t send_key[32], 
                          uint8_t recv_key[32]);
...
int speer_packet_encode(uint8_t* out, size_t* out_len,
                        const uint8_t* in, size_t in_len,
                        const uint8_t cid[SPEER_MAX_CID_LEN], uint8_t cid_len,
                        uint64_t pkt_num,
                        const uint8_t key[32]);
int speer_packet_decode(uint8_t* out, size_t* out_len,
                        const uint8_t* in, size_t in_len,
                        uint8_t cid[SPEER_MAX_CID_LEN], uint8_t* cid_len,
                        uint64_t* pkt_num,
                        const uint8_t key[32]);
...
speer_peer_t* speer_peer_lookup(speer_host_t* host, const uint8_t cid[SPEER_MAX_CID_LEN], uint8_t cid_len);
speer_peer_t* speer_peer_lookup_by_pubkey(speer_host_t* host, const uint8_t pubkey[SPEER_PUBLIC_KEY_SIZE]);
...
int speer_relay_connect(const char* relay_server,
                        const uint8_t local_pubkey[SPEER_PUBLIC_KEY_SIZE],
                        const uint8_t remote_pubkey[SPEER_PUBLIC_KEY_SIZE]);

int speer_noise_xx_initiator_handshake(speer_handshake_t* hs,
                                        const uint8_t* in, size_t in_len,
                                        uint8_t* out, size_t* out_len,
                                        const uint8_t local_privkey[32]);
int speer_noise_xx_responder_handshake(speer_handshake_t* hs,
                                        const uint8_t* in, size_t in_len,
                                        uint8_t* out, size_t* out_len,
                                        const uint8_t local_privkey[32]);

size_t speer_frame_encode_ack(uint8_t* out, uint64_t largest_acked,
                               uint64_t ack_delay, const uint8_t* ranges, size_t num_ranges);
size_t speer_frame_encode_stream(uint8_t* out, uint32_t stream_id,
