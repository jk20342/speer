...
static int encode_peer(speer_pb_writer_t* w, uint32_t field,
    ...
    if (w->pos + inner.pos > w->cap) { w->err = 1; return -1; }
...
static int decode_inner_peer(const uint8_t* in, size_t in_len,
    ...
    while (r.pos < r.len) {
        ...
        if (f == 1 && wire == PB_WIRE_VARINT) {
            ...
            if (opt_peer_id && opt_peer_id_len) {
                ...
                if (decode_inner_peer(d, l, opt_peer_id, &cap) == 0) {
                    *opt_peer_id_len = cap;
                }
            }
