...
static const uint8_t kInitialSalt[20] = {
...
int speer_quic_keys_init_initial(speer_quic_keys_t* ck,
                                   speer_quic_keys_t* sk,
                                   const uint8_t* dcid, size_t dcid_len) {
    ZERO(ck, sizeof(*ck));
    ZERO(sk, sizeof(*sk));
    uint8_t initial_secret[32];
    speer_hkdf2_extract(&speer_hash_sha256, initial_secret,
                         kInitialSalt, sizeof(kInitialSalt),
                         dcid, dcid_len);

    uint8_t client_secret[32], server_secret[32];
    speer_hkdf_expand_label(&speer_hash_sha256, client_secret, 32,
                              initial_secret, 32, "client in", NULL, 0);
    speer_hkdf_expand_label(&speer_hash_sha256, server_secret, 32,
                              initial_secret, 32, "server in", NULL, 0);

    speer_hkdf_expand_label(&speer_hash_sha256, ck->key, 16, client_secret, 32, "quic key", NULL, 0);
    ...
    speer_hkdf_expand_label(&speer_hash_sha256, sk->key, 16, server_secret, 32, "quic key", NULL, 0);
...
int speer_quic_pkt_encode_long(uint8_t* out, size_t out_cap, size_t* out_len,
    ...
    if (p->dcid_len > 0) { COPY(out + pos, p->dcid, p->dcid_len); pos += p->dcid_len; }
    out[pos++] = (uint8_t)p->scid_len;
    if (p->scid_len > 0) { COPY(out + pos, p->scid, p->scid_len); pos += p->scid_len; }
    ...
    if (p->pkt_type == QUIC_PT_INITIAL) {
        ...
        if (p->token_len > 0) { COPY(out + pos, p->token, p->token_len); pos += p->token_len; }
    }
    ...
    if (keys->aead->seal(keys->key, nonce,
                          out, payload_offset,
                          out + payload_offset, p->payload_len,
                          out + payload_offset, tag) != 0) return -1;
...
int speer_quic_pkt_decode_long(speer_quic_pkt_t* p,
    ...
    p->is_long = 1;
    p->version = ((uint32_t)pkt[1] << 24) | ((uint32_t)pkt[2] << 16) |
    ...
    if (keys->aead->open(keys->key, nonce,
                          pkt, hdr_len,
                          pkt + hdr_len, ct_len,
                          pkt + hdr_len + ct_len,
                          pkt + hdr_len) != 0) return -1;
...
int speer_quic_pkt_encode_short(uint8_t* out, size_t out_cap, size_t* out_len,
    ...
    out[pos++] = 0x40
                 ...
                 | ((key_phase & 1) << 2)
                 | (uint8_t)(pn_length - 1);
    if (dcid_len > 0) { COPY(out + pos, dcid, dcid_len); pos += dcid_len; }
    ...
    if (keys->aead->seal(keys->key, nonce,
                          out, payload_offset,
                          out + payload_offset, payload_len,
                          out + payload_offset, tag) != 0) return -1;
