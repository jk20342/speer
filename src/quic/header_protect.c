...
int speer_hp_init(speer_hp_ctx_t* ctx, speer_hp_alg_t alg, const uint8_t* hp_key) {
    ...
    ctx->alg = alg;
    switch (alg) {
        case SPEER_HP_AES_128: ctx->key_len = 16; break;
        case SPEER_HP_AES_256: ctx->key_len = 32; break;
        case SPEER_HP_CHACHA:  ctx->key_len = 32; break;
        default: return -1;
    }
...
int speer_hp_protect(const speer_hp_ctx_t* ctx, uint8_t* pkt, size_t pkt_len,
...
int speer_hp_unprotect(const speer_hp_ctx_t* ctx, uint8_t* pkt, size_t pkt_len,
                        size_t pn_offset, size_t* out_pn_length) {
    size_t sample_off = pn_offset + 4;
    if (sample_off + 16 > pkt_len) return -1;
    uint8_t mask[5];
    if (speer_hp_mask(ctx, pkt + sample_off, mask) != 0) return -1;
