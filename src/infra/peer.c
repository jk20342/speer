...
speer_peer_t* speer_peer_lookup(speer_host_t* host, const uint8_t cid[SPEER_MAX_CID_LEN], uint8_t cid_len) {
    for (speer_peer_t* p = host->peers; p; p = p->next) {
        if (p->conn.cid_len == cid_len &&
            EQUAL(p->conn.cid, cid, cid_len)) {
            return p;
        }
    }
    return NULL;
...
speer_peer_t* speer_peer_lookup_by_pubkey(speer_host_t* host, const uint8_t pubkey[SPEER_PUBLIC_KEY_SIZE]) {
    for (speer_peer_t* p = host->peers; p; p = p->next) {
        if (EQUAL(p->pubkey, pubkey, SPEER_PUBLIC_KEY_SIZE)) {
            return p;
        }
    }
    return NULL;
...
speer_peer_t* speer_peer_create(speer_host_t* host, const uint8_t pubkey[SPEER_PUBLIC_KEY_SIZE]) {
    ...
    if (pubkey) {
        COPY(peer->pubkey, pubkey, SPEER_PUBLIC_KEY_SIZE);
    }
...
speer_stream_internal_t* speer_stream_create(speer_peer_t* peer, uint32_t stream_id) {
    speer_stream_internal_t* s = (speer_stream_internal_t*)calloc(1, sizeof(speer_stream_internal_t));
    if (!s) return NULL;
...
static void peer_check_timeouts(speer_peer_t* peer, uint64_t now_ms) {
    if (peer->state == SPEER_STATE_HANDSHAKE) {
        ...
        if (elapsed > peer->host->config.handshake_timeout_ms) {
            peer->state = SPEER_STATE_CLOSED;
        }
    }
...
void speer_peer_check_all_timeouts(speer_host_t* host) {
    ...
    while (p) {
        ...
        if (p->state == SPEER_STATE_CLOSED) {
            speer_event_t ev = {
                .type = SPEER_EVENT_PEER_DISCONNECTED,
                .peer = p,
