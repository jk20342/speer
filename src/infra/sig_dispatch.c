...
static int parse_ecdsa_der(const uint8_t* sig, size_t sig_len,
                            const uint8_t** r_out, size_t* r_len,
...
int speer_sig_verify_prehashed(uint16_t alg_id,
    ...
    switch (alg_id) {
        case TLS_SIGSCHEME_RSA_PKCS1_SHA256:
        case TLS_SIGSCHEME_RSA_PSS_RSAE_SHA256:
            h = &speer_hash_sha256; break;
        case TLS_SIGSCHEME_RSA_PKCS1_SHA384:
        case TLS_SIGSCHEME_RSA_PSS_RSAE_SHA384:
            h = &speer_hash_sha384; break;
        case TLS_SIGSCHEME_RSA_PKCS1_SHA512:
        case TLS_SIGSCHEME_RSA_PSS_RSAE_SHA512:
            h = &speer_hash_sha512; break;
        case TLS_SIGSCHEME_ECDSA_SECP256R1_SHA256: {
            const uint8_t* pk = pubkey;
            if (pubkey_len == 65 && pubkey[0] == 0x04) pk = pubkey + 1;
            const uint8_t* r; size_t rl; const uint8_t* s; size_t sl;
    ...
    if (is_pss) {
        return speer_rsa_pss_verify(n, n_len, e, e_len, h, msg_hash, msg_hash_len, sig, sig_len, h->digest_size);
    } else {
        return speer_rsa_pkcs1_v15_verify(n, n_len, e, e_len, h, msg_hash, msg_hash_len, sig, sig_len);
    }
}
