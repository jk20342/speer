...
static int init_packet(speer_peer_t* peer, uint32_t stream_id, uint8_t* out, size_t* out_len,
    ...
    if (peer->state != SPEER_STATE_ESTABLISHED) {
        return SPEER_ERROR_HANDSHAKE;
    }
    if (len > SPEER_MAX_PACKET_SIZE - 64) {
        return SPEER_ERROR_BUFFER_TOO_SMALL;
    }
    
    uint8_t frames[SPEER_MAX_PACKET_SIZE];
    size_t frames_len = speer_frame_encode_stream(frames, stream_id, 0, data, len, 0);
    
    return speer_packet_encode(out, out_len,
                              frames, frames_len,
                              peer->conn.cid, peer->conn.cid_len,
                              peer->conn.pkt_num,
                              peer->send_cipher.key);
}

static int process_initial(speer_host_t* host, speer_peer_t* peer,
                         const uint8_t* data, size_t len,
    ...
    speer_event_t ev = {
        .type = SPEER_EVENT_PEER_CONNECTED,
        .peer = peer
    };
...
static int process_handshake_response(speer_host_t* host, speer_peer_t* peer,
                                      const uint8_t* data, size_t len,
                                      const struct sockaddr_storage* addr, socklen_t addr_len) {
    if (len != 32) return -1;
...
static int process_handshake_final(speer_host_t* host, speer_peer_t* peer,
                                   const uint8_t* data, size_t len,
                                   const struct sockaddr_storage* addr, socklen_t addr_len) {
    if (len != 48) return -1;
...
static void buffer_stream_data(speer_peer_t* peer, uint32_t stream_id,
                                const uint8_t* data, size_t len) {
...
static void emit_stream_frames(speer_host_t* host, speer_peer_t* peer, const uint8_t* data, size_t len) {
    size_t pos = 0;
    while (pos < len) {
...
static int process_packet(speer_host_t* host,
                          const uint8_t* data, size_t len,
                          const struct sockaddr_storage* addr, socklen_t addr_len) {
    if (len < 4) return -1;
    ...
    if (type == PACKET_TYPE_INITIAL) {
        if (!peer) {
            ...
            return process_handshake_response(host, peer, data + hdr_len, len - hdr_len,
                                              addr, addr_len);
        }
        ...
        if (peer && peer->state == SPEER_STATE_HANDSHAKE) {
            ...
            if (body_len == 48) {
                return process_handshake_final(host, peer, data + hdr_len, body_len, addr, addr_len);
            }
        }
        ...
        if (speer_packet_decode(plaintext, &plaintext_len,
                               data, len,
                               cid, &cid_len, &pkt_num,
...
speer_host_t* speer_host_new(const uint8_t seed_key[SPEER_PRIVATE_KEY_SIZE],
    ...
    host->socket = speer_socket_create(host->config.bind_port,
...
speer_stream_t* speer_stream_open(speer_peer_t* peer, uint32_t stream_id) {
    ...
    if (!s) {
        ...
        speer_event_t ev = {
            .type = SPEER_EVENT_STREAM_OPENED,
            .peer = peer,
            .stream = (speer_stream_t*)s,
            .stream_id = stream_id
        };
        if (peer->host->callback) {
...
void speer_stream_close(speer_stream_t* stream) {
    ...
    if (s) {
        ...
        speer_event_t ev = {
            .type = SPEER_EVENT_STREAM_CLOSED,
            .peer = stream->peer,
            .stream = stream,
            .stream_id = stream->id
        };
        if (stream->peer->host->callback) {
            stream->peer->host->callback(stream->peer->host, &ev, 
                                         stream->peer->host->user_data);
        }
    }
...
int speer_stream_write(speer_stream_t* stream, const uint8_t* data, size_t len) {
    ...
    int n = speer_socket_send(peer->host->socket, packet, packet_len,
                              &peer->addr, peer->addr_len);
