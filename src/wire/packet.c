...
static size_t encode_header(uint8_t* out, uint8_t type,
...
static size_t decode_header(const uint8_t* in, size_t in_len,
                            uint8_t* type,
...
size_t speer_frame_encode_crypto(uint8_t* out, uint64_t offset,
...
size_t speer_frame_encode_connection_close(uint8_t* out, uint64_t error_code,
...
int speer_frame_parse(const uint8_t* in, size_t in_len,
    ...
    while (parser_avail(&p) > 0) {
        ...
        if (on_frame(type, p.buf + start, p.pos - start, user) != 0) {
            return -1;
        }
    }
