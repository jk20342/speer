...
int speer_pb_read_bytes(speer_pb_reader_t* r, const uint8_t** data, size_t* len) {
    ...
    if (!pb_avail(r, (size_t)l)) { r->err = 1; return -1; }
...
int speer_pb_skip(speer_pb_reader_t* r, uint32_t wire) {
    switch (wire) {
        case PB_WIRE_VARINT: { uint64_t v; return speer_pb_read_varint(r, &v); }
        case PB_WIRE_64BIT:  { if (!pb_avail(r, 8)) { r->err = 1; return -1; } r->pos += 8; return 0; }
        case PB_WIRE_32BIT:  { if (!pb_avail(r, 4)) { r->err = 1; return -1; } r->pos += 4; return 0; }
        case PB_WIRE_LEN:    { const uint8_t* d; size_t l; return speer_pb_read_bytes(r, &d, &l); }
...
int speer_pb_write_bytes_field(speer_pb_writer_t* w, uint32_t field, const uint8_t* data, size_t len) {
    if (speer_pb_write_tag(w, field, PB_WIRE_LEN) != 0) return -1;
    if (speer_pb_write_varint(w, (uint64_t)len) != 0) return -1;
    if (w->pos + len > w->cap) { w->err = 1; return -1; }
