...
static void mdns_format_ipv4(char out[16], uint32_t addr_be) {
    ...
    snprintf(out, 16, "%u.%u.%u.%u", (unsigned)b[0], (unsigned)b[1], (unsigned)b[2], (unsigned)b[3]);
}
...
static size_t decode_name(const uint8_t* pkt, size_t pkt_len, size_t offset,
...
int mdns_init(mdns_ctx_t* ctx) {
    ...
    setsockopt(ctx->socket_ipv4, IPPROTO_IP, IP_ADD_MEMBERSHIP,
...
int mdns_build_announcement(uint8_t* out, size_t* out_len, const mdns_service_t* svc) {
    ...
    for (uint32_t i = 0; i < svc->txt.num_fields; i++) {
        char txt_field[256];
        int len = snprintf(txt_field, sizeof(txt_field), "%s=%s",
                          svc->txt.fields[i].key, svc->txt.fields[i].value);
        if (len > 255) len = 255;
        out[pos++] = (uint8_t)len;
...
int mdns_announce(mdns_ctx_t* ctx) {
    ...
    for (uint32_t i = 0; i < ctx->num_services; i++) {
        ...
        if (mdns_build_announcement(packet, &len, &ctx->services[i]) == 0) {
            ...
            sendto(ctx->socket_ipv4, (const char*)packet, (int)len, 0,
...
int mdns_poll(mdns_ctx_t* ctx, int timeout_ms) {
    ...
    int received = 0;
    while (1) {
        int n = recvfrom(ctx->socket_ipv4, (char*)ctx->recv_buffer,
                         MDNS_MAX_PACKET_SIZE, 0,
        ...
        char peer_id[128];
        char multiaddr[256];
        if (mdns_parse_packet(ctx, ctx->recv_buffer, (size_t)n,
