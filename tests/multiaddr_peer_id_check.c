...
int main(void) {
    ...
    if (speer_multiaddr_parse(&ma, "/ip4/192.168.0.42/tcp/4001") != 0)
        FAIL("multiaddr parse ip4/tcp\n");

    char host[64];
    uint16_t port = 0;
    if (speer_multiaddr_to_host_port_v4(&ma, host, sizeof(host), &port) != 0 || port != 4001 ||
        strcmp(host, "192.168.0.42") != 0)
        FAIL("to_host_port_v4\n");

    char s[256];
    if (speer_multiaddr_to_string(&ma, s, sizeof(s)) != 0 ||
        strcmp(s, "/ip4/192.168.0.42/tcp/4001") != 0)
    ...
    if (speer_multiaddr_parse(
            &mc,
            "/ip4/10.0.0.1/tcp/1/p2p-circuit/p2p/QmAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") !=
        0)
    ...
    uint8_t proto[128];
    size_t proto_len = 0;
    if (speer_libp2p_pubkey_proto_encode(proto, sizeof(proto), SPEER_LIBP2P_KEY_ED25519, ed,
                                         sizeof(ed), &proto_len) != 0)
    ...
    uint8_t peerid[64];
    size_t peerid_len = 0;
    if (speer_peer_id_from_pubkey_bytes(peerid, sizeof(peerid), proto, proto_len, &peerid_len) !=
            0 ||
