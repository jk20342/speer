...
static int fuzz_mdns_parse(const uint8_t *data, size_t size, mdns_header_t *hdr,
                           mdns_record_t *recs, int max_rec) {
    if (size < 12) return -1;
    ...
    for (int i = 0; i < hdr->answers && pos < size && rec_count < max_rec; i++) {
        ...
        r->type = (data[pos] << 8) | data[pos + 1];
        pos += 2;
        r->class = (data[pos] << 8) | data[pos + 1];
        pos += 2;
        r->ttl = ((uint32_t)data[pos] << 24) | ((uint32_t)data[pos + 1] << 16) |
                 ((uint32_t)data[pos + 2] << 8) | data[pos + 3];
        pos += 4;
        r->rdlength = (data[pos] << 8) | data[pos + 1];
        pos += 2;
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        uint8_t valid[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x04, 't',  'e',  's',  't',  0x05, 'l',  'o',
                           'c',  'a',  'l',  0x00, 0x00, 0x01, 0x00, 0x01};
