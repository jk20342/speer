...
static uint8_t local_pubkey[32] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
                                   0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
                                   0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20};

static uint8_t local_privkey[32] = {0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28,
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    if (size >= 32) {
        ...
        if (size >= 48) { speer_noise_xx_read_msg3(&hs, data); }
    }
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        uint8_t buf[256] = {0};
        for (size_t i = 0; i <= 256; i++) { LLVMFuzzerTestOneInput(buf, i); }
